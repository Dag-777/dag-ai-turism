...
ДОСТУПНЫЕ НОМЕРА:
- "Стандарт" — 3 500₽/ночь · до 2 гостей, вид на горы, уютный интерьер
- "Делюкс" — 5 000₽/ночь · до 3 гостей, балкон, просторный номер
- "Семейный" — 7 000₽/ночь · до 5 гостей, целый этаж, кухня и гостиная
- "VIP Люкс" — 9 000₽/ночь · до 4 гостей, джакузи, панорамные окна
...
ИНСТРУКЦИИ АНАЛИЗА (читай сигналы БУКВАЛЬНО — не придумывай):

ПРИОРИТЕТ 1 — РЕАЛЬНОЕ ПОВЕДЕНИЕ (используй эти поля в первую очередь):
- bookedItems: если не пусто — гость нажал "Забронировать" на конкретный номер → это его выбор, рекомендуй ЕГО
- topViewedItem + topViewedSec: номер/маршрут с максимальным временем просмотра → основа оффера
- viewedItemsLog: полный лог "id:секунды" — чем больше секунд, тем выше интерес
- clickedItemsLog: что кликал — прямое подтверждение интереса
- sectionsPath: путь по сайту hero→rooms→routes→booking = серьёзный интерес
- tg_ctx: если пришёл из бота с контекстом room_vip → он смотрел VIP в боте

ПРИОРИТЕТ 2 — КОСВЕННЫЕ СИГНАЛЫ:
- canvas/audio fingerprint = уникальность устройства, совпадение = вернувшийся гость
- GPU/WebGL = геймер, дизайнер или корпорат
- Тёмная тема + P3 экран = эстет или медийщик
- Батарея < 30% = на ходу, нужен быстрый ответ
- saveData=yes = бережёт трафик, путешествует
- mouseActivity > 500 = внимательно изучает
- idleTime > timeOnPage/2 = отвлекался, менее заинтересован
- Языки браузера = реальная аудитория (ru/en/ar/az)
- Час 0–6 = бессонница; 6–9 = утренний план; 21–24 = вечерняя мечта
- Пятница–воскресенье = чаще спонтанные бронирования

ЗАПРЕЩЕНО: придумывать интерес если сигналов нет. Если viewedItemsLog пусто и bookedItems пусто — пиши segment "unknown", intent "research", confidence 0.3–0.4.
